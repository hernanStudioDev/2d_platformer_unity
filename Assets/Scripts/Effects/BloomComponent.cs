...
namespace SideScroller01
{
    public class BloomComponent : DrawableGameComponent
    {
        ...
        public void UpdateBloom(float end, GameTime gT)
        {
            float calcBloom;

            if (Settings.BloomIntensity < end) // Zooming In
            {

                calcBloom = (end - Settings.BloomIntensity) * BloomSettings.BaseSpeed * BloomSettings.Elasticity;
                IncrementBloom(calcBloom);

                if (Settings.BloomIntensity > end)
                    Settings.BloomIntensity = end;

            }
            else
            {
                calcBloom = (Settings.BloomIntensity - end) * BloomSettings.BaseSpeed * BloomSettings.Elasticity;
                DecreaseBloom(calcBloom);

            }

        }

        public void IncrementBloom(float amount)
        {
            Settings.BloomIntensity = Settings.BloomIntensity + amount;
        }

        public void DecreaseBloom(float amount)
        {
            Settings.BloomIntensity = Settings.BloomIntensity - amount;
        }

        public void SetBloom(float bloomIntensity, float speed)
        {
            Game1.bloomIntensityIndex = bloomIntensity;
            BloomSettings.BaseSpeed = speed;
        }
